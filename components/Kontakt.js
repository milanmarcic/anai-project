      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="container px-5 py-24 mx-auto flex">
          <div className="lg:w-1/3 md:w-1/2 bg-white  p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
            <h2 className="text-gray-900 text-lg mb-1 font-medium title-font text-center">Kontaktirajte nas</h2>

            <div className="relative mb-4 text-red-600">
              <label className="leading-7 text-sm text-gray-600">Ime i prezime</label>
              <input {...register("firstAndLastName", { required: true })} type="text" className="w-full bg-white rounded border border-gray-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
              {errors.firstAndLastName?.type === 'required' && "Molimo unesite ime i prezime"}
            </div>
            <div className="relative mb-4 text-red-600">
              <label {...register("eMail", { required: true })} type="text" className="leading-7 text-sm text-gray-600">E-mail</label>
              <input className=" w-full bg-white rounded border border-gray-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
              {errors.eMail?.type === 'required' && "Molimo unesite e-mail"}
            </div>
            <div className="relative mb-4 text-red-600">
              <label className="leading-7 text-sm text-gray-600">Poruka</label>
